...
describe('Триал (#47) — первая игра бесплатно', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('не выставляет инвойс если это первая игра (countUserSessions = 0)', async () => {
  ...
  it('создаёт сессию с бюджетом в одну звезду (millicentsPerStar) при триале', async () => {
    vi.mocked(sessionsRepo.countUserSessions).mockResolvedValue(0);
  ...
  it('сохраняет триал-платёж со статусом paid и суммой 0', async () => {
    vi.mocked(sessionsRepo.countUserSessions).mockResolvedValue(0);
    vi.spyOn(Telegram.prototype as any, 'callApi').mockResolvedValue({});
  ...
  it('отправляет стартовое сообщение с нарративом игры при триале', async () => {
    vi.mocked(sessionsRepo.countUserSessions).mockResolvedValue(0);
    const callApi = vi.spyOn(Telegram.prototype as any, 'callApi').mockResolvedValue({
      message_id: 10,
    });
  ...
  it('выставляет инвойс если это НЕ первая игра (countUserSessions > 0)', async () => {
  ...
  it('запускает новую игру тестеру бесплатно даже если это не первая игра', async () => {
    vi.mocked(usersRepo.upsertUser).mockResolvedValue(tester);
    vi.mocked(sessionsRepo.countUserSessions).mockResolvedValue(3);
    const callApi = vi.spyOn(Telegram.prototype as any, 'callApi').mockResolvedValue({});
    ...
    expect(paymentsRepo.createPayment).toHaveBeenCalledWith(
      expect.objectContaining({
        amount: 0,
        status: 'paid',
        payload: expect.objectContaining({ tester: true, sessionId: trialSession.id }),
      }),
    );
  });
});
